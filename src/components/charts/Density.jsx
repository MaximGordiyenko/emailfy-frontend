...
export const Density = ({ width, height, data }) => {
  ...
  const densityGenerator = kernelDensityEstimator(kernelEpanechnikov(2), xScale.ticks(40));
  ...
  const allYMax = densityData.map((group) => Math.max(...group.density.map((d) => d[1])));
